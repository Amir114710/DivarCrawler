...
class DivarSpider(scrapy.Spider):
    ...
    def parse(self , reponse):
        ...
        parking = False if 'ندارد' in informations[3].extract() else True
        elevator = False if 'ندارد' in informations[3].extract() else True

        address = reponse.css('div div.kt-page-title__subtitle--responsive-sized').extract()
        ...
        image = reponse.css('picture img.kt-image-block__image--fading').extract()

        discription = reponse.css('div p.kt-description-row__text--primary::text').extract()

        phone = reponse.css('div div.expandable-box::text').extract_first()
